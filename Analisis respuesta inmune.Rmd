...
```{r}
datos_largo <- datos %>% select(ID, Lugar, SEXO, Edad_calculada, Vacuna, ANT_COVID, GrupoEdad, baseline, after_dosis1, after_dosis2) %>% 
...
datos_largo1 <- as.data.frame(datos_largo1) %>% arrange(momento, ID) %>% 
                mutate(ANT_COVID2 = if_else(ANT_COVID == "NO" & momento == "baseline" & titulo > 0, "SI", ANT_COVID))
```

### Tabla univariado (linea de base)
...
Los gráficos donde se comparan los títulos de anticuerpos en cada momento parten de observaciones completas para las extracciones de base y posteriores a cada dosis de vacuna (n = 658).
...
conteo <- datos1 %>% filter(!is.na(ANT_COVID2)) %>% count(ANT_COVID)
